...
import { VmiBinsTableProps } from "@insite/content-library/Widgets/VmiBins/VmiBinsTable";
...
export const DispatchBeginExportVmiProducts: HandlerType = props => {
    props.dispatch({
        type: "Pages/VmiBins/BeginExportVmiProducts",
    });
};
...
export const ExportData: HandlerType = async props => {
    ...
    const tableWidget = pageWidgets?.find(o => o.type === "VmiBins/VmiBinsTable");
    ...
    if (Object.keys(props.parameter.ids).length) {
        const vmiBinsDataView = getVmiBinsDataView(state, state.pages.vmiBins.getVmiBinsParameter);
        ...
        const result = await getVmiBins({
            ...state.pages.vmiBins.getVmiBinsParameter,
            page: 1,
            pageSize: 9999,
            expand: ["product"],
        });
...
export const DispatchCompleteExportVmiProducts: HandlerType = props => {
    props.dispatch({
        type: "Pages/VmiBins/CompleteExportVmiProducts",
    });
};
