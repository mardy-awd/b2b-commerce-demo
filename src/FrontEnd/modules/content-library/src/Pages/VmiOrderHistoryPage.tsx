...
import resetSelectedOrders from "@insite/client-framework/Store/Pages/OrderHistory/Handlers/ResetSelectedOrders";
...
const mapStateToProps = (state: ApplicationState) => ({
    ...
    orderHistory: state.pages.orderHistory,
});
...
const VmiOrderHistoryPage = ({
    ...
    useEffect(() => {
        setVmiOrderHistoryPage(true);
        firstLoad = true;
        const pageSizeCookie = getCookie("VmiOrderHistory-PageSize");
        ...
        if (location.search) {
            ...
            let fromDate;
            if (settings.orderSettings.lookBackDays > 0) {
                const tzOffset = new Date().getTimezoneOffset() * 60000;
                fromDate = new Date(Date.now() - settings.orderSettings.lookBackDays * 60 * 60 * 24 * 1000 - tzOffset);
            }
            updateSearchFields({
...
const pageModule: PageModule = {
    component: withHistory(connect(mapStateToProps, mapDispatchToProps)(VmiOrderHistoryPage)),
    definition: {
...
/**
 * @deprecated Use string literal "VmiOrderHistoryPage" instead of this constant.
 */
export const VmiOrderHistoryPageContext = "VmiOrderHistoryPage";
