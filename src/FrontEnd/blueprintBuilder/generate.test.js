...
test("Should find three errors", () => {
    const result = parseErrorsFromOutput(
        `ERROR in /src/insite-commerce/modules/blueprints/genericName/src/Components/SocialLinks.tsx
ERROR in /src/insite-commerce/modules/blueprints/genericName/src/Components/SocialLinks.tsx(144,25):
...
ERROR in /src/insite-commerce/modules/blueprints/genericName/src/Overrides/Widgets/Header/MainNavigation.tsx
ERROR in /src/insite-commerce/modules/blueprints/genericName/src/Overrides/Widgets/Header/MainNavigation.tsx(513,26):
...
ERROR in /src/insite-commerce/modules/blueprints/genericName/src/Overrides/Widgets/Header/MainNavigation.tsx
ERROR in /src/insite-commerce/modules/blueprints/genericName/src/Overrides/Widgets/Header/MainNavigation.tsx(532,38):
...
npm ERR! errno 1
npm ERR! root@ node-build:server: \`node node_modules/webpack/bin/webpack.js --config=config/webpack/prodServer.js "--env" "BLUEPRINT=genericName"\`
...
ERROR: "node-build:server -- --env BLUEPRINT=genericName" exited with 1.
npm ERR! code ELIFECYCLE
npm ERR! errno 1
npm ERR! root@ build:server: \`npm-run-all "node-build:server -- --env BLUEPRINT={1}" -- "genericName"\`
npm ERR! Exit status 1
npm ERR! 
...
ERROR: "build:server genericName" exited with 1.
npm ERR! code ELIFECYCLE
npm ERR! errno 1
npm ERR! root@ build: \`npm-run-all "build:server {1}" "build:client {1}" -- "genericName"\`
npm ERR! Exit status 1
npm ERR! 
...
`,
    ...
    expect(result[0].message)
        .toBe(`TS2322: Type '{ iconProps: { size: number; css: FlattenSimpleInterpolation; src: string | React.NamedExoticComponent<IconPresentationProps> | undefined; }; position: "left"; }' is not assignable to type '{ iconProps?: IconPresentationProps | undefined; position?: "left" | "right" | undefined; }'.
  The types of 'iconProps.src' are incompatible between these types.
    Type 'string | NamedExoticComponent<IconPresentationProps> | undefined' is not assignable to type 'string | ComponentClass<{}, any> | FunctionComponent<{}> | undefined'.
      Type 'NamedExoticComponent<IconPresentationProps>' is not assignable to type 'string | ComponentClass<{}, any> | FunctionComponent<{}> | undefined'.
        Type 'NamedExoticComponent<IconPresentationProps>' is not assignable to type 'FunctionComponent<{}>'.
    ...
    expect(result[2].message).toBe(
        `TS2741: Property 'displayChangeCustomerLink' is missing in type '{ index: number; link: MappedLink; styles: MainNavigationStyles; container: RefObject<HTMLDivElement>; isOpen: boolean; }' but required in type 'Pick<ThemeProps<BaseTheme> & OwnProps & RefAttributes<MainNavigationItem>, "link" | "ref" | "key" | ... 4 more ... | "container">'.`,
    );
